...
public class FirstController {
    ...
    public FirstDTO queryAll(Integer uid,String type,String subType){
        FirstDTO firstDTO = firstService.queryAll(uid, type, subType);
        return firstDTO;
    }

    @RequestMapping("queryDetailByArticle")
    public Article queryDetailByArticle(Integer uid,Integer id){
        Article article = firstService.queryArticleById(id);
        return article;
    }

    @RequestMapping("queryDetailByAlbum")
    public Album queryDetailByAlbum(Integer id,Integer uid){
        Album album = firstService.queryAlbumById(id);
        return album;
    }

    @RequestMapping("login")
    public Object login(String phone,String password){
        Object obj = firstService.login(phone, password);
        return obj;
    }
    @RequestMapping("regist")
    public Object regist(String phone,String password){
        Object obj = firstService.regist(phone, password);
        return obj;
    }
    @RequestMapping("updateMsg")
    public Object updateMsg(User user){
        Object obj = firstService.updateMsg(user);
        return obj;
    }
    //获取短信验证码
    @RequestMapping("getCode")
    public void getCode(String phone) throws ClientException {
        System.out.println(phone+"传入的电话号码----");
        //设置超时时间-可自行调整
...
//请求成功
    ...
    public List<User> getAllMember(String uid){
        List<User> allMember = firstService.getAllMember(uid);
        return allMember;
    }
}
